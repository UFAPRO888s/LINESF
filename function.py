...
from CHANGYEDNOOK import TalkService
from CHANGYEDNOOK.ttypes import *
import time, json, requests, os, random, ast, datetime, sys, concurrent.futures,re
...
class CHANGYEDNOOK:
    def __init__(self, myToken, myApp, pool=False):
        ...
        self.groups = self.openTransport("/CH4")
        self.profile = self.getProfile()
        self.serverTime = self.getServerTime()
        self.groups = self.getAllChatMids()
        self.localRev = -1
        self.globalRev = 0
    ...
    def sendMessageCustom(self,to,text,mids):
        ...
        annda={"MSG_SENDER_NAME": name,"MSG_SENDER_ICON": pict}
        self.sendMessage(to, text, contentMetadata=annda)
